...
import com.coderzxh.web.config.ResponseHelper;
...
//                responseError(request, response);
    ...
    private void setUserBean(ServletRequest request, ServletResponse response, JWTToken token) {
        ...
        int userId =  JWTUtil.getUserCode(token.getPrincipal().toString());
        User userBean = userService.selectById(userId);
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (httpServletRequest.getRequestURI().equals("/camera/images/favicon-16x16.png") || httpServletRequest.getRequestURI().equals("/camera/images/favicon-32x32.png")) {
            httpServletResponse.setStatus(HttpStatus.OK.value());
            return false;
        }
        logger.info("into-url:  " + httpServletRequest.getRequestURI());
    ...
    private boolean verificationPassAnnotation(ServletRequest request, ServletResponse response, HttpServletRequest httpServletRequest, String authorization) throws Exception {
        for (String urlMethod: Constant.METHOD_URL_SET) {
            ...
            if(split[0].equals(requestURI)
                    && (split[1].equals(method) ||  split[1].equals("RequestMapping")))
            {
                ThreadLocalUtil.setIsPass(true);
                if(ComUtil.isEmpty(authorization)){
    ...
    private void responseError(ServletRequest request, ServletResponse response) {
        ...
        try {
            ...
            logger.error("error-url:  " + httpServletRequest.getRequestURI());
            out.print(JSONObject.toJSONString(ResponseHelper.validationFailure(PublicResultConstant.UNAUTHORIZED + httpServletRequest.getRequestURI())));
            out.flush();
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
